...
export default function FileUpload({ onUploadSuccess, onUploadError, accept = 'image/*,.pdf,.doc,.docx' }) {
  ...
  const loadStorageInfo = async () => {
    try {
      ...
      if (response.success && response.data?.storage) {
        ...
        setStorageInfo({ used: storage.used || 0, limit: storage.limit || 0, available: available, percentage: percentage });
      }
    } catch (err) {
    ...
    <div className="space-y-4">
      {storageInfo && (
        <div className="text-sm font-medium text-gray-700 bg-blue-50 border border-blue-200 rounded-lg px-4 py-2">
          ...
          {storageInfo.available < 1024 * 1024 && <span className="ml-2 text-red-600 font-semibold">Low storage!</span>}
        </div>
      )}
      <div className="flex items-center gap-4">
        <label className="cursor-pointer">
          <input type="file" accept={accept} onChange={handleFileSelect} disabled={uploading} className="hidden" />
          <span className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:bg-gray-400">
        ...
        {file && (
          <>
            ...
            <button onClick={handleUpload} disabled={uploading} className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600 disabled:bg-gray-400">Upload</button>
          </>
        )}
      </div>
      {preview && (
