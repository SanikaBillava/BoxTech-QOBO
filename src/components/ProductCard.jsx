...
export default function ProductCard({ product }) {
  ...
  return (
    <Link to={`/products/${product.slug}`} className="group block bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-secondary">
      ...
      <div className="p-4">
        ...
        {product.ply_type && (
          <span className="inline-block text-xs bg-secondary text-white font-medium px-2 py-1 rounded mb-2">{product.ply_type}</span>
        )}
      </div>
    </Link>
  );
